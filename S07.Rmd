...
As you notice, here we set `alternative="two.sided"`, this means we are testing whether the two population means are different (one could be smaller or larger than the other), and this requires a two-sided p-value. We set `var.equal = T` when call the `t.test()` function. By doing so, we assume an "equal SD" method, please see page `40` in [R.S] for more. By default, R uses the "unequal SD" method, which returns a slightly different t statistic and d.f. We will look at this in the future.

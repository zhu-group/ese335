---
title: "Section 07 One-sample Z-test and t-test"
output:
  html_document:
    toc: true
    toc_float:
...
So 4 out of 5 of these numbers are higher than `70`, and some of them by quite a bit. Can this be explained on the basis of chance variation? Or does it show bias?

***

# The Z-Ratio

We also know that the spectrophotometer can measure CO concentration up to 100 ppm with an instrument error of `10` ppm (1 $\sigma$). Instrument error generally follows a normal distribution. In this case, let's now assume that the error follows a normal distribution $N(\mu , 10)$, where $\mu$ is unknown. With this set-up, we can write:
...
In this case, $Estimate$ is the sample average ($\overline{X}$), and $Parameter$ is the population mean ($\mu$). Since the sample if from the population with a known SD ($\sigma$), the SD of average ($\overline{X}$) is also $\sigma$. 

For our job, it is then specified as:

$$Z=\frac {\overline{X} - \mu} {\sigma}  $$

Basically, Z-Ration describes  how far do the observations deviate from the assumed mean. 

***

# One sample Z-test

In this case, $\overline X$ is `7.8`, as the mean of `8, 13, -2, 2, 18`,  and $\sigma$ is `10`. Assuming H0 is true, we get $Z=0.78$.

If the sampling distribution of the estimate is normal, then the sampling distribution of $Z$ follows standard distribution by definition. Then with R, we can compute the p-value:
...
OK, we have a probability of about `21.77%` getting a statistic ($Z$) as extreme or more extreme than the observed statistic (`0.78`), assuming H0 is true. This is not a small probability, and is not likely due to chance. We can not reject H0 given the observations. Thus, the instrument has no bias based on the observations.

Now change the instrument error to `2.0` and repeat the above steps. Can you explain what happens?
...
Here the *Standard Error* (SE) refers to the sample. If $s$ is the sample SD, the SE for the average is 

$$SE(\overline {X})=\frac {s} {\sqrt{n}}$$
And we have:

$$t=\frac {\overline{X} - \mu} { {s} / {\sqrt{n}}}  $$
If $\overline {X}$ is the average in a random sample of size $n$ from a normal population, the sampling   distribution of its t-Ratio follows a Student's t-distribution on $n-1$ degrees of freedom (df). 
...
In this case, $\overline X$ is again `7.8`, $SE(\overline X)$ is Assuming H0 is true, we get $t=2.16$.
...
Now, we have a probability of about `4.84%` getting a statistic ($t$) as extreme or more extreme than the observed statistic (`2.16`), assuming H0 is true. This is a small probability, and is likely due to chance. We can reject H0 given the observations. Thus, the instrument is positively biased based on the observations.

***

# One sample t-test with R

It turns out you can perform the previous one-sample t-test simaply as:

+ H0: The instrument has no bias ($\mu = 70$)

+ H1: The instrument has a positive bias ($\mu > 70$)

In R simply, this is done by:

---
title: "Section 11 One-way ANOVA"
output:
  html_document:
    toc: true
    toc_float:
...
Cadmium (Cd) has been identified as the priority control metal in China due to its higher concentrations in soil and higher health risks posed to the public. [^1]

[^1]: Chen et al., Contamination features and health risk of soil heavy metals in China, *Science of The Total Environment*, **512–513**, p.143-153, 2015.
...
Do soil Cd concentrations diff under the four conditions? Or do different plants influence soil Cd concentration differently? How to test this? Can we use the t-tools from previous sections?
...
One-way ANOVA is similar to the independent samples t-test. The later one is used for comparison of population means across **two levels**, while one-way ANOVA is used for the comparison of **more than two** population means. The logic of ANOVA is essentially the same as the t-test - it compares variability between groups to variability within groups to determine whether the observed differences are due to chance or not.
...
In this case, the *F*-statistic is `4.66`, assuming H0 is true ($\mu_D = 0$). Then the p-value can be calculated manually:
...
Here we put `var.equal = T` when call the `oneway.test` function. By doing so, we assume equal SD in the populations, similar to the “equal SD” method used in the independent two-sample t-test. By default, R uses unequal SD in populations (`var.equal = F`), which becomes the Welch test and which returns a different *F* statistic and d.f. We will look at this in the future.

Another way to perform the one-way ANOVA with R is calling the `aov()` and `summary()` function. By doing so, R returns the full ANOVA table (df~B~, df~W~, SS~B~, SS~W~, MSB, MSW, *F*, and p-value) and you can also save the ANOVA for post analysis.
...
An environmentalist wanted to determine if the mean acidity of rain differs among `3` sites. He randomly selected `6` rain dates at each site obtained the following data:

+ Site #1: `5.11, 5.01, 4.90, 5.14, 4.80, 5.24`

+ Site #2: `4.87, 4.18, 4.40, 4.67, 4.89, 4.09`

+ Site #3: `5.46, 6.29, 5.57, 5.15, 5.45, 5.30`

So, does the mean acidity of rain differ among the `3` sites?
...
[^2]: Example and data are from Shu Tao, *Applied statistical methods*, p. 246-247.

---
title: "Section 20 Plotting with R (II): making maps"
output:
  html_document:
    toc: true
    toc_float:
...
From here we can see that there are `925` polygons, and R just follow the polygons defined by this object then line them each one up. If you want to see the data hidden in `SpatialPolygonsDataFrame`, we can use the operator `@`:
...
# Name in Chinese
# This is the same as the NAME in original shape file
...
There are many ways of plotting an interactive map. Here we show you how to do this with the `leafletCN` package, you are welcome to explore other tools.
...
Suppose we want to plot site locations on a city scale, and we want to add a base map which shows road, terrain, and built-up areas. This can be done with the `ggmap()` package:
...
# Pull the base map, you may need a VPN
...
Download the [path of Typhoon Mangkhut](https://zhu-group.github.io/ese335/download/Mangkhut.txt), and read via:
...
Plot a map of your home city (zoom=15), and add a few points on to the map.

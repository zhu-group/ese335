---
title: "Section 06 Normal distribution"
output:
  html_document:
    toc: true
    toc_float:
...
This section starts with a poll. Suppose we want to know how many hours SUSTech undergraduates generally sleep every day. Let's further assume that ESE335 students are representative of the whole population.

Now count the number of students for each of the following intervals:
...
There are several possibilities to check normality, including visual inspections, such as normal plots/histograms,  Q-Q (quartile-quartile) plot, and statistical tests, such as Sapiro-Wilk test and Lillifors (Kolmogorov–Smirnov) test.
...
The Sapiro-Wilk method is widely used to check normality of a **small sample** (size less than `30`). In R, the test is done with `shapiro.test()`.
...
```{r}
# Sapiro-Wilk test of sample 4
shapiro.test(sample4)
```

## Lillifors test

The Kolmogorov-Smirnov (K-S) test is used to test whether or not or not a sample comes from a certain distribution. The Lillifors test is an improvement on the K-S test — correcting the K-S for small values at the tails of probability distributions — and is therefore sometimes called the K-S D test. The Lillifors test can be conducted with the `lillie.test()` function from the `nortest` package:

```{r}
# Lillifors test of sample 1
lillie.test(sample1)

# Lillifors test of sample 2
lillie.test(sample2)
```

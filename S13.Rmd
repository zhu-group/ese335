---
title: "Section 13 Covariance and Pearson correlation test"
output:
  html_document:
    toc: true
    toc_float:
...
$$r_{xy} = \frac{\sum_{i}^{n} (x_i - \overline{x})(y_i - \overline{y})} {\sqrt{\sum_{i}^{n} (x_i - \overline{x})} \sqrt{\sum_{i}^{n} (y_i - \overline{y})} }   $$
In theory, if (*X*,*Y*) follows a bivariate normal:

$$\frac {r \sqrt{n-2}}{\sqrt{1-r^2}} \sim t_{df=n-2}$$

With this, we can test the significance of the observed Pearson correlation coefficient.

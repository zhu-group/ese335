...
The normal distribution is the most important probability distribution in statistics because it fits many natural phenomena. For example, heights, blood pressure, measurement error, and IQ scores follow the normal distribution. The reason why normal distribution is so "normal" in nature can be traced to the **Central Limit Theory**, which states that various **independent** factors influence a particular trait (see [Exercise #2 from Section 02](https://zhu-group.github.io/ese335/S03.html#Exercise_2) for more). When all these independent factors contribute to a phenomenon, their normalized sum tends to result in a normal distribution.

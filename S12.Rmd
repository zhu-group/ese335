---
title: "Section 12 Two-way ANOVA and post-hoc test"
output:
  html_document:
    toc: true
    toc_float:
...
We saw from the last section that one-way ANOVA is used to test whether means among populations are the same. Once a one-way ANOVA is done, and significant differences are identified, one would be more interested in finding out which specific groups’s means (compared with each other) are different. The test compares all possible pairs of means. And this is so-called the post-hoc test of one-way ANOVA. Here we introduce the *Tukey Test* (or Tukey procedure), also called Tukey’s Honest Significant Difference test, which is based on the studentized range distribution. 

Let's use the Cd example from the last section for the one-way ANOVA and its Tukey test.
...
Now suppose we would like to test whether other approaches could have an impact on soil Cd concentration. Instead of using plants (#1, #2, and #3), we also try chemical (with chemical #1 and #2) and physical (#1, #2, #3, and #4) method. We measure Cd concentration from each combination. 
...
Similar to the one-way ANOVA, we can use the Tukey test lets us see which groups are different from one another.
...
+ The red line representing the mean of the residuals should be **horizontal and centered on zero** (or on one, in the scale-location plot), meaning that there are no large outliers that would cause bias in the model.

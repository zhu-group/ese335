...
Assuming H0 is true, we get $Z$ is `-2.16`. Then with R, we can compute two-side p-value for the statistic:
...
print(P_value)
```
OK, we have a probability of about `3.06%` getting a statistic ($Z$) as extreme or more extreme than the observed statistic (`-2.16`), assuming H0 is true. This is a small probability, and is likely due to chance. We can reject H0 given the observations. Thus, mean PM2.5 level in Shenzhen is not the same as that in Guangzhou.

***

# Independent two-sample t-test

As we saw from the last Section, SD of populations ($\sigma_1$ and $\sigma_2$) are generally unknown. And we need to use sample SE as an estimate of the population SD, and the the proceeding Z test becomes the t test:

$$ Z=\frac { (\overline{X_1} - \overline{X_2} ) - (\mu_1 - \mu_2) } {SE(\overline{X_1} - \overline{X_2}) } $$
...
Let's use the observations again, but conduct an independent two-sample t-test. 
...
Now, we have a probability of about `2.80%` getting a statistic ($t$) as extreme or more extreme than the observed statistic (`-2.37`), assuming H0 is true. This is a small probability, and is likely due to chance. We can reject H0 given the observations. Thus,  mean PM2.5 level in Shenzhen is not the same as that in Guangzhou.
...
```
As you may notice, here we set `var.equal = T` when call the `t.test()` function. By doing so, we assume an "equal SD" method, please see page `40` in [R.S] for more. By default, R use the "unequal SD" method, which would returns a slightly different t statistic and d.f. We will look at this in the future.
...
A local environmental officer wants to compare two rivers to see if there are any difference in COD. From river A, the officer randomly collected `15` water bottles, and COD are measured as follows:

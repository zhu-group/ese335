---
title: "Section 16 Simple linear regression (II)"
output:
  html_document:
    toc: true
    toc_float:
...
| Uptaken amount                      | Pesticide concentration in soil |
|:------------------------------------|---------------------------------|
| `0.15, 0.16, 0.18, 0.19, 0.20, ...` |       `10`                      |
| `0.98, 1.00, 1.01, 1.05, 1.10, ...` |       `50`                      |
| `0.38, 0.34, 0.49, 0.41, 0.50, ...` |       `20`                      |
| `0.57, 0.61, 0.65, 0.70, 0.66, ...` |       `30`                      |
| `1.34, 1.67, 1.74, 1.58, 1.88, ...` |       `80`                      |
| `1.11, 1.12, 1.33, 1.10, 1.21, ...` |       `60`                      |
| `1.36, 1.37, 1.41, 1.50, 1.60, ...` |       `70`                      |
| `0.74, 0.77, 0.78, 0.79, 0.77, ...` |       `40`                      |
...
print(P_value)
```
For simple linear regression, notice that the p-values are identical. This is not a coincidence; this is based on the mathematical relationship between a t-distribuion with *ν* df and the *F*-distribution with (1, *ν*) df.
...
+ **2. Independence**. The *Y* values are statistically independent of each other. This is often not valid when *X* variable is time, which shows a feature called *auto-regression*. In this case, use *time-series analysis* methods. The assumption can be tested with the [runs test](https://zhu-group.github.io/ese335/S10.html#Independence). 
